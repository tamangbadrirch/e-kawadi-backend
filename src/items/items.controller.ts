...
export class ItemsController {
  ...
  @Get('/category/:id')
  async itemByCategoryId(@Param('id') id: string, @Res() response: Response) {
    try {
      const itemList = await this.itemsService.findByCategoryId(id);
      return response.send({
        data: itemList,
        message: 'successfully get item from category id',
      });
    } catch (e) {
      response
        .send({ data: null, message: 'Internal server error' })
